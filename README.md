# 👋 Hello, I'm Nallakaruppannasamy

### 📡 Electronics and Communication Engineering | 💻 MERN Stack Developer | 🚀 Executive Council Member @ ISTE Sathyabama

I am a passionate and curious B.E. ECE student at **Sathyabama Institute of Science and Technology**. I specialize in blending my technical hardware background with creative software problem-solving to build responsive, full-stack web applications.

## 🛠️ Tech Stack

* **Frontend:** React.js, Next.js, HTML5, CSS3, JavaScript (ES6+)
* **Backend:** Node.js, Express.js, REST APIs
* **Database:** MongoDB, Oracle
* **Other Tools:** Git, GitHub, Socket.IO, UI/UX Design Principles

## 🌟 What I'm up to

* 🏗️ **Building:** Developing a custom e-commerce web app inspired by modern development practices.
* 🔭 **Exploring:** Deep-diving into Real-time apps using **Socket.IO** and advanced Authentication.
* 🤝 **Collaborating:** Actively contributing to the **ISTE Student Chapter** at Sathyabama as an Executive Council Member.
* 🌐 **Deploying:** Learning the ropes of Cloud deployment and version control for scalable apps.

## 🏆 Community & Leadership

* **Executive Council Member @ ISTE:** Leading technical initiatives and fostering a growth mindset within the student community.
* **Event Organizer:** Previously organized high-impact technical events like *Code Crusaders* and *Tech Compass*.

## 🌈 Beyond Coding

* ⚡ Blending ECE hardware concepts with software logic.
* 💡 Brainstorming new ways to improve user experience in web applications.
* 🤝 Mentoring peers in web development fundamentals.

## 📫 Let's Connect!

Feel free to reach out if you want to collaborate on MERN projects or discuss the latest in tech!

[**LinkedIn**](https://www.linkedin.com/in/nallakaruppannasamyr) | [**Portfolio**](https://my-portfolio-rose-pi-46.vercel.app)


## 📊 GitHub Stats

<p align="left">
  <img src="https://github-readme-stats.vercel.app/api?username=Nallakaruppannasamy&show_icons=true&theme=radical" alt="GitHub Stats" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Nallakaruppannasamy&layout=compact&theme=radical" alt="Top Languages" />
</p>

## 🛡️ Tech Stack Badges

![React](https://img.shields.io/badge/react-%2320232a.svg?style=for-the-badge&logo=react&logoColor=%2361DAFB)
![NodeJS](https://img.shields.io/badge/node.js-6DA55F?style=for-the-badge&logo=node.js&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-%234ea94b.svg?style=for-the-badge&logo=mongodb&logoColor=white)
![JavaScript](https://img.shields.io/badge/javascript-%23323330.svg?style=for-the-badge&logo=javascript&logoColor=%23F7DF1E)
![TailwindCSS](https://img.shields.io/badge/tailwindcss-%2338B2AC.svg?style=for-the-badge&logo=tailwind-css&logoColor=white)
